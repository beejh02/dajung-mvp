...
- [ ] 메뉴 목록 API 구현
- [ ] 메뉴 상세 API 구현
- [ ] 주문 생성 시 메뉴/옵션 기준 서버 가격 재계산 구현
- [ ] 필수 옵션/선택 개수/비활성 메뉴 옵션 검증 구현
- [ ] 주문 생성 API 구현
- [ ] 주문 상세 API 구현
- [ ] 내 주문 목록 API 구현
- [ ] 주문 상태 전이 검증 구현
- [ ] `PATCH /orders/{order_id}/status` 관리자/내부 전용 처리
- [ ] 더미 결제 승인 API 구현
- [ ] 결제 승인 후 주문 상태 변경/포인트 적립/영수증 생성을 단일 트랜잭션으로 처리
- [ ] 결제 승인 API idempotency 구현
- [ ] 포인트 적립 중복 방지 구현
- [ ] 영수증 중복 생성 방지 구현
- [ ] 주문 실패/결제 실패 더미 케이스 정의
- [ ] 백엔드 서비스 단위 테스트 작성: 가격 재계산, 옵션 검증, 상태 전이, idempotency
...
- [ ] 영수증 발급 상태 표시
- [ ] 주문 출처별 통계 표시
- [ ] 키오스크 주문과 AI Agent 주문이 함께 반영되는지 확인
